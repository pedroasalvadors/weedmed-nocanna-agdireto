              ...
              <div className={`absolute bottom-0 left-0 right-0 p-4 flex flex-col items-start transition-opacity duration-300 md:opacity-0 md:group-hover:opacity-100 ${isActive ? 'opacity-100' : 'opacity-0 md:opacity-0'}`}>
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    // Open Leadster chat
                    const neurolead = (window as unknown as { neurolead?: { open: () => void } }).neurolead
                    if (neurolead) {
                      neurolead.open()
                    }
                  }}
                  className="open-chat px-4 py-2 text-sm font-semibold text-white rounded-full shadow-md hover:shadow-lg transition-all duration-300"
                  style={{
